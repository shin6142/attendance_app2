...
class SlackApiDriver(@Autowired private val env: Environment) {
    fun fetchMessages(employeeId: Int, year: String, month: String): Either<SlackApiError, SlackApiResponse> = either {
        val token = env.getProperty("slack.token")
        val authToken = "Bearer $token"
        val userId = env.getProperty("slack.userId")
        val before = "${year}-${month}-31"
        val after = "${year}-${month}-01"
        val query = URLEncoder.encode(
            "from:<$userId> in:grp-dev-勤怠 before:$before after:$after",
